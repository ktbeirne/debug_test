...
1. 記事の作成・取得・更新・削除
2. コメントの投稿
3. 記事一覧の取得（フィルタ・ページネーション）

### 異常系（バグを仕込む箇所）

1. **削除済み記事へのアクセス** → `ART-002`を返すべきだが、実装漏れでnull参照エラー
2. **存在しない著者IDでの記事作成** → `USR-001`を返すべきだが、実装漏れでundefined参照エラー
3. **他人の記事の編集** → `ART-004`を返すべきだが、権限チェック漏れで編集可能
4. **存在しない記事へのコメント投稿** → `CMT-001`を返すべきだが、チェック漏れでエラー
5. **tagsがnullの記事の取得** → 配列操作でエラー
...
Lambda Function (Node.js 22.x)
  ↓
CloudWatch Logs
  ↓ (Subscription Filter: severity = "ERROR")
